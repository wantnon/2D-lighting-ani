...
void main() {
    ...
    float Hscale=1.0/255.0*hScale;
    
    float Hdn=texture2D(texHeightMap,ddn).b*Hscale;
    float Hrt=texture2D(texHeightMap,dright).b*Hscale;
    float Hc=texture2D(texHeightMap,dcenter).b*Hscale;
