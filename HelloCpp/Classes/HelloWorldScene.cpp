...
*/
    ...
    for(int i=0;i<frameCount;i++){
        ...
        if(picIndex>9){//i is two-digit number
            ...
            sprintf(fileName_height, "actor_fish_shayu_normal_0%i_h.png",picIndex);

        }else{//i is one-digit number
            sprintf(fileName, "actor_fish_shayu_normal_00%i.png",picIndex);
            sprintf(fileName_height, "actor_fish_shayu_normal_00%i_h.png",picIndex);
        }
        CCTexture2D*texture=CCTextureCache::sharedTextureCache()->addImage(fileName);
        CCTexture2D*texture_height=CCTextureCache::sharedTextureCache()->addImage(fileName_height);
        sceneNode->heightTexMap[texture]=texture_height;
    ...
    sceneNode->runAction(CCRepeatForever::create(CCAnimate::create(animation)));
    ...
    //----height scale slider
	{
        CCSize screenSize = CCDirector::sharedDirector()->getWinSize();
        //title
        CCLabelTTF* pLabel = CCLabelTTF::create("height scale  ", "Arial", 30);
        uiNode_lighting->addChild(pLabel, 1);
		//
		CCControlSlider *slider = CCControlSlider::create("sliderTrack.png","sliderProgress.png" ,"sliderThumb.png");
		slider->setAnchorPoint(ccp(0.5f, 0.5f));
        ...
        m_initValueA=pSceneNode->getHScale();
		slider->setValue(m_initValueA);
		...
		uiNode_lighting->addChild(m_pSliderCtlA,100);
		//
        pLabel->setPosition(m_pSliderCtl->getPosition()+ccp(0,-m_pSliderCtl->getContentSize().height/2-pLabel->getContentSize().height/2));
        slider->setPosition(pLabel->getPosition()+ccp(0,-pLabel->getContentSize().height/2-slider->getContentSize().height/2));
    ...
    // author info
    {
        CCLabelTTF* pLabel = CCLabelTTF::create("by yang chao (wantnon) 2013-11-15", "Arial", 30);
    ...
    CCLabelTTF* pLabel = CCLabelTTF::create("2D lighting with animation", "Arial", 45);
...
void HelloWorld::menu0Callback(CCObject* pSender){
    ...
    checkPic->setPosition(ccp(pMenuItem0->getContentSize().width+checkPic->getContentSize().width/2,pMenuItem0->getContentSize().height/2));
    //
    pSceneNode->setUseLightingEffect(false);
...
void HelloWorld::menu1Callback(CCObject* pSender){
    ...
    //
    pSceneNode->setUseLightingEffect(true);
